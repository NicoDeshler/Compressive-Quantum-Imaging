function PersonickWaveletEstimation_CumLikelihood(array_id)
...
[img,~] = gen_wavelet_sparse_img(WaveletName,WaveletLevel,img_dims,q);
...
z_min = 1e-10;                                     % min variance
z_max = 2e-3;                                      % max variance
a_mu0 = zeros(n_as,1);
a_mu1 = zeros(n_as,1);
...
save_dir = fullfile('Testing','8x8_recon_CumulativeLikelihood');
...
iter = 1;
l_vec_stack = zeros(N_HG_modes+1,max_iter);
B_gamma_stack = zeros(N_HG_modes, N_HG_modes);
B_gamma_stack(:,:,1) = B_gamma;
ref_mu = a_mu_GBM;
ref_sigma = a_cov_GBM;
while iter <= max_iter
    ...
    l_vec_stack(:,iter) = l_vec;
    
    
    % sample the posteriors and estimate the wavelet coefficients
   [a_vec,a_mu_post,a_cov_post] = BayesianUpdate_CumLikelihood(...
                                                  l_vec_stack, B_gamma_stack,C_vec,...
                                                  a_mu0, a_cov0, a_mu1, a_cov1, q,...
                                                  N_samples,ref_mu,ref_sigma,...
                                                  W,wv_idx,WaveletName);
        
    
    % set parameters in importance sampling's proposal distribution for next iteration
    ref_mu = a_mu_post;
    ref_sigma = a_cov_post;
